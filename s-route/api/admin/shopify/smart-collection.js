...
// [REQUIRE] Personal //
const rh_admin_shopify_smartCollections = require('./smart-collection.handler.js')
...
router.post(
	...
	async (req, res) => {
		res.send(
			await rh_admin_shopify_smartCollections.rh_update({ req })
		)
	}
)
...
router.get(
	...
	async (req, res) => {
		res.send(
			await rh_admin_shopify_smartCollections.rh_delete_all({ req })
		)
	}
)
