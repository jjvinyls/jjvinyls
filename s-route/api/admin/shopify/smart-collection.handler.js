...
module.exports = {
	...
	remove_condition_filter: async ({ req }) => {
		...
		const sleeve = JSON.stringify({
			column: 'tag',
			relation: 'equals',
			condition: 'sleeve-Mint (M)'
		})

		const result = await api_shopify.a_list_all({
			limit: req.params.limit,
			page: req.params.page,
		})
		...
		for (let i = 0; i < result.collections.length; i++) {
			const c = result.collections[i];
			
			let newRules = []

			for (let ii = 0; ii < result.collections[i].rules.length; ii++) {
				const r = result.collections[i].rules[ii]

				if (
					JSON.stringify(r) !== mint &&
					JSON.stringify(r) !== sleeve
				) { newRules.push(r) }
			}
